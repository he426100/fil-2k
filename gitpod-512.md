...
docker run --rm \
  ...
  pre-seal --sector-size=512MiB --num-sectors=2
```
...
docker run -d -it \
  ...
  -e DOCKER_LOTUS_MINER_INIT_ARGS="--genesis-miner --actor=t01000 --sector-size=512MiB --pre-sealed-sectors=/data/.genesis-sectors --pre-sealed-metadata=/data/.genesis-sectors/pre-seal-t01000.json --nosync" \
  -v /workspace/tmp/fil-2k-data:/data \
...
docker run -d -it \
  ...
  -e DOCKER_LOTUS_MINER_INIT=true \
  -e DOCKER_LOTUS_MINER_INIT_ARGS="--sector-size=512MiB" \
  -e RUST_LOG=info \
  -v /workspace/tmp/fil-2k-data:/data \
  -v /workspace/tmp/filecoin-proof-parameters:/var/tmp/filecoin-proof-parameters \
  --gpus all \
